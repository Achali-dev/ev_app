...
import com.example.smart_ev.Modals.Battery;
...
public class Home extends AppCompatActivity {
    ...
    private Runnable mBatteryStatusRunnable = new Runnable() {
        @Override
        public void run() {
            getBatteryStatus();
            mHandler.postDelayed(this, 3000); // Fetch battery status every 1 second
        }
    };
    ...
    protected void onResume() {
        super.onResume();
        mHandler.post(mBatteryStatusRunnable); // Start fetching battery status when the activity is resumed
    }

    @Override
    protected void onPause() {
        super.onPause();
        mHandler.removeCallbacks(mBatteryStatusRunnable); // Stop fetching battery status when the activity is paused
    }

    private void getBatteryStatus() {
        Call<Battery> call = RetrofitClient.getInstance().getApi().get_status();
        call.enqueue(new Callback<Battery>() {
            ...
            public void onResponse(Call<Battery> call, Response<Battery> response) {
                if (response.isSuccessful())
                {
                    ...
                    Battery battery1 = response.body();
                    percentage.setText(battery1.getStatus()+"%");
                    percentage.setTextColor(Color.GREEN);
                    double batteryStatus = Double.parseDouble(battery1.getStatus());

                    // Compare battery status
                    if (batteryStatus < 50) {
                        percentage.setTextColor(Color.YELLOW);
                        if (batteryStatus >= 49 && batteryStatus <=50) {
                            playAudio();
                        }
                    ...
                    if (batteryStatus <= 25) {
                        ...
                        if (batteryStatus >= 24 && batteryStatus <= 25) {
                            playAudio();
                        }
                    }
                    if (batteryStatus < 5) {
    ...
    private void playAudio() {
        MediaPlayer mediaPlayer = MediaPlayer.create(getApplicationContext(), R.raw.battery_low_now);
        mediaPlayer.start();
    }
